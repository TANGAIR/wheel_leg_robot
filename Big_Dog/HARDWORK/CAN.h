...
typedef struct
{
	...
	//发送
  struct 
	{ //T=send_kp*send_Position+send_kd*send_Velocity+send_torque
	  float send_Position;   //rad  -4*pi and 4*pi
	  float send_Velocity;   //rad/s -30 and + 30 rad/s
	  float send_kp;         //max:500
	  float send_kd;         //max:5
	  float send_torque;	   //nm -18 and 18 N-m
	}send;
	
	//接收
	struct
	{
		float recieve_Position;  //-12-12RAD
		float recieve_Velocity; //1500RPM 78RAD/S
		float recieve_Current;	//17A   -40 AND 40
	}recieve;
