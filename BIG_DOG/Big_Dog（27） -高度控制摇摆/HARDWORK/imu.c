...
void IMU_Get_Data_Task(void *pvParameters)
{
  ...
  float RC_Joint_Y_Ra=0.045454;
	...
	while(1)
	{
     ...
     Left_Front_Leg_Struct.Y_Add   = (DBUS.RC.ch3-DBUS.RC.ch2)*RC_Joint_Y_Ra;// KI*LEFT_RIGHT_WIDTH*sin(IMU_Real_Data.Pitch*PI_180) - KI*FRONT_BACK_LENGTH*sin(IMU_Real_Data.Roll*PI_180 );
     Right_Front_Leg_Struct.Y_Add  = (DBUS.RC.ch3+DBUS.RC.ch2)*RC_Joint_Y_Ra;// KI*LEFT_RIGHT_WIDTH*sin(IMU_Real_Data.Pitch*PI_180) + KI*FRONT_BACK_LENGTH*sin(IMU_Real_Data.Roll*PI_180 );
     Left_Back_Leg_Struct.Y_Add    = (-DBUS.RC.ch3-DBUS.RC.ch2)*RC_Joint_Y_Ra;//-KI*LEFT_RIGHT_WIDTH*sin(IMU_Real_Data.Pitch*PI_180) - KI*FRONT_BACK_LENGTH*sin(IMU_Real_Data.Roll*PI_180 );
     Right_Back_Leg_Struct.Y_Add   = (-DBUS.RC.ch3+DBUS.RC.ch2)*RC_Joint_Y_Ra;//-KI*LEFT_RIGHT_WIDTH*sin(IMU_Real_Data.Pitch*PI_180) + KI*FRONT_BACK_LENGTH*sin(IMU_Real_Data.Roll*PI_180 );


		if(Left_Front_Leg_Struct.Y_Add>=30) Left_Front_Leg_Struct.Y_Add=30;
		if(Left_Front_Leg_Struct.Y_Add<=-30)  Left_Front_Leg_Struct.Y_Add=-30;
		
    if(Right_Front_Leg_Struct.Y_Add>=30) Right_Front_Leg_Struct.Y_Add=30;
		if(Right_Front_Leg_Struct.Y_Add<=-30)  Right_Front_Leg_Struct.Y_Add=-30;

    if(Left_Back_Leg_Struct.Y_Add>=30) Left_Back_Leg_Struct.Y_Add=30;
		if(Left_Back_Leg_Struct.Y_Add<=-30)  Left_Back_Leg_Struct.Y_Add=-30;

    if(Right_Back_Leg_Struct.Y_Add>=30) Right_Back_Leg_Struct.Y_Add=30;
		if(Right_Back_Leg_Struct.Y_Add<=-30)  Right_Back_Leg_Struct.Y_Add=-30;			
