...
/**
  ...
  * @param  void
  * @retval void
  * @notes  串口6打印初始化    USART1_TX-PA9      USART1_RX -----PB7
  */
...
/* ------------------------------------- Configure DMA  接收---------------------------------------*/
  USART_DMACmd(USART1,USART_DMAReq_Rx,ENABLE);
	...
	DMA_InitStruct.DMA_Memory0BaseAddr=(uint32_t)USART_CAN_Rx;                    //存储器数据地址
...
void DMA2_Stream7_IRQHandler(void)
{
	if(DMA_GetITStatus(DMA2_Stream7,DMA_IT_TCIF7)==SET)
	{
	 ...
	 DMA_Cmd(DMA2_Stream7,DISABLE);
		...
		{
	   DMA_InitStruct.DMA_Memory0BaseAddr   =(uint32_t)USART_CAN_Tx;                 //存储变量
	   DMA_Init(DMA2_Stream7,&DMA_InitStruct);
	   DMA_change_flag =1;
		}		
...
void USART1_IRQHandler(void)
{
	...
	if(USART_GetITStatus(USART1,USART_IT_IDLE)==SET)
	{
     ...
     num = USART1->DR; //清除串口空闲中断（中文手册712页）
		 ...
		 DMA_Cmd(DMA2_Stream5,DISABLE); 
		 DMA_SetCurrDataCounter(DMA2_Stream5,15);      //重新设置接收数据长度
...
void USART12CAN_TX_Send_Cmd (char CAN_DATA_CMD[8],char ID)
{
	...
	Tx_massage.Data_Lenth  = 0x08;                                                     //帧长度为8
	Tx_massage.Can_ID  = (ID<<21)&0xfff00000 ;                                                      //帧ID为接收信息的电机的ID
	...
	Tx_massage.Data[4] = CAN_DATA_CMD[4];               //203接收电流高8位
	Tx_massage.Data[5] = CAN_DATA_CMD[5];                  //203接收电流低8位
	Tx_massage.Data[6] = CAN_DATA_CMD[6];               //204接收电流高8位
	Tx_massage.Data[7] = CAN_DATA_CMD[7];                  //204接收电流低8位
...
void USART12CAN_TX_Jiont (CAN1_Data_TypeDef* motor)
{
	...
	Tx_massage.Data_Lenth = 0x08;                                                                      //帧长度为8
	Tx_massage.Can_ID  = (motor->Motor_ID<<21)&0xfff00000;                                             //帧ID为电机ID
	...
	Tx_massage.Data[4] = (float_to_uint(motor->send.send_kp, KP_MIN, KP_MAX, 12))&0xff;                     //203接收电流高8位
	Tx_massage.Data[5] = (float_to_uint(motor->send.send_kd, KD_MIN, KD_MAX, 12)>>4)&0xff;                  //203接收电流低8位
	Tx_massage.Data[6] = (float_to_uint(motor->send.send_kd, KD_MIN, KD_MAX, 12)<<4)|((float_to_uint(motor->send.send_torque, T_MIN, T_MAX, 12)>>8)&0x0f);               //204接收电流高8位
	Tx_massage.Data[7] = (float_to_uint(motor->send.send_torque, T_MIN, T_MAX, 12) )&0xff;                  //204接收电流低8位
