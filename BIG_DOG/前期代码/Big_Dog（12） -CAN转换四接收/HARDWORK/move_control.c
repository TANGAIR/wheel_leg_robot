  ...
  Foot_Direction_Init();    //腿部位置初始化，收缩状态
	...
	{	
		...
		if(mode==1)
		{
			  ...
			  USART12CAN_TX_Send_Cmd(CAN_DATA_CMD_ON,Contron_ID);
				vTaskDelay(delay_time);
				USART12CAN_TX_Send_Cmd(CAN_DATA_CMD_ON,Contron_ID_2);
				vTaskDelay(delay_time);
...
//		}
		...
		{
	  	  USART12CAN_TX_Send_Cmd(CAN_DATA_CMD_OFF,Contron_ID);
				vTaskDelay(delay_time);
			  USART12CAN_TX_Send_Cmd(CAN_DATA_CMD_OFF,Contron_ID_2);
			  vTaskDelay(delay_time);
