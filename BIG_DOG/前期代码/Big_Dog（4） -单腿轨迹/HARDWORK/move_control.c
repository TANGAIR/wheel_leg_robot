...
//步态控制标志位
 ...
 void signal_leg(Leg_Data_Typedef *Leg_Data)
 {
   ...
   if(TIM_t<=STEP_FORWARD_TIME)
	 ...
	 else if(TIM_t<=(STEP_FORWARD_TIME+QUIESECENCE_TIME+20))
	 {
...
void Move_Control_Task(void *pvParameters)
{
	...
	Foot_Direction_Init();    //腿数据初始化
  Jiont_Motor_FOC_Init();   //关节电机参数初始化,启动所有电机
	...
	{
		//
		//Leg_Diretion_Control(&Right_Front_Leg_Struct,foot_x,foot_y);
...
//		 CAN1_Send_Cmd(CAN_DATA_CMD_OFF,7);
//	   CAN1_Send_Cmd(CAN_DATA_CMD_OFF,8);
