...
{
  MPU6500_NSS_Low();
  BMI088_read_write_byte(reg&0x7f);//spi传输协议：每次读写操作需要16个时钟周期或更多，传输的第一个字节为SPI地址，第二个字节为SPI数据。首字节的第一位为读写位，0位写，1为读，后面7位为寄存器地址
...
uint8_t MPU6500_Read_Reg(uint8_t  reg)
{
  ...
  MPU_Rx=BMI088_read_write_byte(0xff);
  MPU6500_NSS_High();
