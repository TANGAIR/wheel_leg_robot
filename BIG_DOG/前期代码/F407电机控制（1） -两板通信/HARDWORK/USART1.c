     ...
     num = USART6->DR; //清除串口空闲中断（中文手册712页）
		...
		if((C_To_F407_Rx[0]==0x0A)&&(C_To_F407_Rx[17]==0x0B))
		{
			...
			Joint_RF_D.send.send_Position  =  uint_to_float((C_To_F407_Rx[15]<<8)|(C_To_F407_Rx[16]), P_MIN, P_MAX, 16) ; 
				 ...
				 //通过串口发送信息，控制各腿
//		 {
//				USART22CAN_TX_Jiont(&Joint_LF_U);
//			  USART22CAN_TX_Jiont(&Joint_LF_D);
//			 
//				USART32CAN_TX_Jiont(&Joint_LB_U);
//			  USART32CAN_TX_Jiont(&Joint_LB_D);
//			 
				 if(turn_flag==1)USART42CAN_TX_Jiont(&Joint_RB_U);
			   else USART42CAN_TX_Jiont(&Joint_RB_D);	
			  
//			 
//				USART52CAN_TX_Jiont(&Joint_RF_U);
//			  USART52CAN_TX_Jiont(&Joint_RF_D);
//		 
//		 }
