...
void Coordinate_Inverse(Leg_Data_Typedef* Leg_Data)
{
	...
	//X限位防止出错，X工作空间在-200~200
		if(Leg_Data->Leg_X<=-200)Leg_Data->Leg_X=-200;
		if(Leg_Data->Leg_X>=200)Leg_Data->Leg_X=200;
	//Y限位防止出错，Y工作空间在200~400
	   if(Leg_Data->Leg_Y>=400)Leg_Data->Leg_Y=400;
		 if(Leg_Data->Leg_Y<=200)Leg_Data->Leg_Y=200;
...
/**
  ...
  */
 void Foot_Dynamic_Control(Leg_Data_Typedef* Leg_Data )
{
	//
	 double fai=0;
	...
	//抬腿前进部分
     ...
     //数据类型转换		 
		 ...
		 //轨迹坐标计算
		 Leg_Data->Leg_X =step_lenght/2/pi*(fai-sin(fai))+FOOT_X;
		 Leg_Data->Leg_Y =(STEP_HIGHT/2*(1-cos(fai)))*(-1)+Leg_Data->FOOT_y+Leg_Data->Y_Add;
  ...
  Leg_Struct_Point->step_forward_time=0;
	...
	switch(Leg_Struct_Point->Leg_Type)
	{
		...
		case LEFT_FRONT_LEG:
		{
			 ...
			 Leg_Struct_Point->FOOT_y = FOOT_Y-30 ;
				break;
		}
				//左后
		case LEFT_BACK_LEG:
