...
float  LF_U_Last_Position=0,
			 LF_D_Last_Position=0,//左前下角度电机2
			 LB_U_Last_Position=0,//左后上角度电机3
			 LB_D_Last_Position=0,//左后下角度电机4
			 RB_U_Last_Position=0,//右后上角度电机5
			 RB_D_Last_Position=0,//右后下角度电机6
			 RF_U_Last_Position=0,//右前上角度电机7
			 RF_D_Last_Position=0;//右前下角度电机8
	...
	if(USART_GetITStatus(USART6,USART_IT_IDLE)==SET)
	{
     ...
     num = USART6->DR; //清除串口空闲中断（中文手册712页）
		...
		if((C_To_F407_Rx[0]==0x0A)&&(C_To_F407_Rx[17]==0x0B))
		{
			...
			//限位
			if(Joint_LF_U.send.send_Position>2)Joint_LF_U.send.send_Position=2;
			if(Joint_LF_U.send.send_Position<-2)Joint_LF_U.send.send_Position=-2;
			
			//Joint_LF_U.send.send_Position=-1;
			
			//双环PID计算
			Joint_LF_U.Motor_Angle_PID ->PIDout =Pid_Calc(Joint_LF_U.Motor_Angle_PID ,Joint_LF_U.recieve.recieve_Position,Joint_LF_U.send.send_Position );
		
			Joint_LF_U.send .send_torque = Pid_Calc(Joint_LF_U.Motor_Speed_PID ,Joint_LF_U.recieve.recieve_Velocity ,Joint_LF_U.Motor_Angle_PID ->PIDout) ;
