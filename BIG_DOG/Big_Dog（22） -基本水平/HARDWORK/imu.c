...
void IMU_Get_Data_Task(void *pvParameters)
{
	...
	while(1)
	{
     ...
     Left_Front_Leg_Struct.Y_Add   = 0;// KI*LEFT_RIGHT_WIDTH*sin(IMU_Real_Data.Pitch*PI_180) - KI*FRONT_BACK_LENGTH*sin(IMU_Real_Data.Roll*PI_180 );
     Right_Front_Leg_Struct.Y_Add  = 0;// KI*LEFT_RIGHT_WIDTH*sin(IMU_Real_Data.Pitch*PI_180) + KI*FRONT_BACK_LENGTH*sin(IMU_Real_Data.Roll*PI_180 );
     Left_Back_Leg_Struct.Y_Add    = 0;//-KI*LEFT_RIGHT_WIDTH*sin(IMU_Real_Data.Pitch*PI_180) - KI*FRONT_BACK_LENGTH*sin(IMU_Real_Data.Roll*PI_180 );
     Right_Back_Leg_Struct.Y_Add   = 0;//-KI*LEFT_RIGHT_WIDTH*sin(IMU_Real_Data.Pitch*PI_180) + KI*FRONT_BACK_LENGTH*sin(IMU_Real_Data.Roll*PI_180 );


		if(Left_Front_Leg_Struct.Y_Add>=50) Left_Front_Leg_Struct.Y_Add=50;
		if(Left_Front_Leg_Struct.Y_Add<=-50)  Left_Front_Leg_Struct.Y_Add=-50;
		
    if(Right_Front_Leg_Struct.Y_Add>=50) Right_Front_Leg_Struct.Y_Add=50;
		if(Right_Front_Leg_Struct.Y_Add<=-50)  Right_Front_Leg_Struct.Y_Add=-50;

    if(Left_Back_Leg_Struct.Y_Add>=50) Left_Back_Leg_Struct.Y_Add=50;
		if(Left_Back_Leg_Struct.Y_Add<=-50)  Left_Back_Leg_Struct.Y_Add=-50;

    if(Right_Back_Leg_Struct.Y_Add>=50) Right_Back_Leg_Struct.Y_Add=50;
		if(Right_Back_Leg_Struct.Y_Add<=-50)  Right_Back_Leg_Struct.Y_Add=-50;			
