...
char left_up=106;           //变大向内，向右偏，YAW为正向左偏
char right_up=180;          //变小向内
 float kp=1.2;
 //IMU
 //抬头时，PITHCE为负数
...
void Move_Control_Task(void *pvParameters)
{
  ...
  Steering_Init();//舵机初始化，头舵机设置为90°
	...
	while(1)
	{
	 ...
	 Left_Front_Speed(left_up )     //W ，数字越小越向外   + (IMU_Real_Data.YAW/3>15?15:(IMU_Real_Data.YAW/3))
	 Left_Back_Speed(left_up)	     //X     + (IMU_Real_Data.YAW/3>15?15:(IMU_Real_Data.YAW/3))
   Right_Front_Speed(right_up)   //Y ，数字越大越向外
	 Right_Back_Speed(right_up)		 //A
		
		
		
		Left_Front_Leg_Struct.Y_Add = kp*(IMU_Real_Data.Pitch<-25?-25:IMU_Real_Data.Pitch) ;
		Right_Front_Leg_Struct.Y_Add = kp*(IMU_Real_Data.Pitch<-25?-25:IMU_Real_Data.Pitch);
		Right_Back_Leg_Struct.Y_Add = -kp*(IMU_Real_Data.Pitch<-25?-25:IMU_Real_Data.Pitch) ;
		Right_Back_Leg_Struct.Y_Add = -kp*(IMU_Real_Data.Pitch<-25?-25:IMU_Real_Data.Pitch);
...
//		}
//		if(TIM_tt==80)TIM_tt=0;
//		
			//坐标调试
		
		 Leg_Diretion_Control(&Left_Front_Leg_Struct,L_F_x,L_F_y);
		 Leg_Diretion_Control(&Left_Back_Leg_Struct,L_B_x,L_B_y);
		 Leg_Diretion_Control(&Right_Front_Leg_Struct,R_F_x,R_F_y);
		 Leg_Diretion_Control(&Right_Back_Leg_Struct,R_B_x,R_B_y);
...
void Shrink(void)
{
	 ...
	 Left_Front_Speed(left_up)    //W   
	 Left_Back_Speed(left_up)	    //X
   Right_Front_Speed(right_up)   //Y
	 Right_Back_Speed(right_up)		//A
